...
LogicalResult ViterbiDecodeOp::verify() {
  ...
  if (std::optional<std::string> broken =
          checkViterbiFrame(getConstraintLength(), getPolynomials(), symbolsType.getDimSize(0),
                            bitsType.getDimSize(0)))
    return emitOpError(*broken);
  if (std::optional<int64_t> bound = getSymbolBound())
    if (*bound < 1 || *bound > 32767)
      return emitOpError("symbol_bound must lie in [1, 32767]");
  return success();
}
