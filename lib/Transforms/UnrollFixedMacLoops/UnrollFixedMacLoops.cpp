...
/// Static macs this loop leaves behind, or nothing when it is not simple
/// enough to price. Cost is what the transform PRODUCES, since a trip count
/// alone undercounts a body an earlier pass already expanded.
///
/// A guard region is priced like any other operation, because unrolling clones
/// it once per iteration. A nested LOOP is refused instead: pricing it exactly
/// means modelling what unrolling does inside it, and no lowering here emits
/// that shape, so the refusal costs nothing real.
std::optional<int64_t> getUnrolledMacCost(scf::ForOp loop) {
  int64_t macs = 0;
  bool nestedLoop = loop.getBody()
                        ->walk([&](Operation *op) {
                          if (isa<scf::ForOp>(op))
  ...
  if (!trip)
    return macs;
  int64_t total;
  if (llvm::MulOverflow(*trip, macs, total))
    return std::nullopt;
  return total;
}
