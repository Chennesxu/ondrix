...
| Full-output FIR | Experimental tensor-only destination-style `ondrix.fir_filter` for valid and zero-padded full boundaries; valid supports output-axis tiling, while full uses guarded scalar edges plus a Q15/Q31 Vectorized interior; ordered f32, dynamic shape guards, `K > N`, alias-safe AOT execution, and direct bufferization are covered; same boundaries, full-output tiling origins, public buffer semantics, fusion, target-aware selection, and streaming remain open |
