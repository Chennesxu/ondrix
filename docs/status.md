...
fixed full-output FIR, static factor-two Q15 FIR decimation, valid
...
| FIR interpolation | Experimental tensor-only `ondrix.fir_interpolate` defines factor-two phase-zero interpolation as the complete convolution of a zero-inserted input, with result length `(N-1)*2+K` and increasing coefficient update order; one signed-Q15/full i40 profile reaches generic scalar object+C differential execution; source binding, stateful block forms, polyphase scheduling, f32/Q31, Vector, and target consumers remain unsupported |
...
| Object generation and C execution | Implemented for scalar and fixed-width Vector Q15/Q31 FIR-sample/full-output paths, factor-two Q15 FIR decimation including its `.ox` binding, factor-two Q15 scalar FIR interpolation, valid Q15/f32 convolution/correlation, ordered scalar Q15/Q31/f32 streaming chunks, opt-in materialized Q15/Q31 streaming Vector reuse, the experimental f32 TDF-II plus uniform-Q fixed DF-II SOS cascades, the closed packed-Q15 scalar/Vector butterfly and static four-/eight-point scalar/Vector forward/inverse CFFT profiles, and the `.ox` Q15/Q31/f32 dot/FIR, Q15 streaming FIR/fixed SOS, and packed-Q15 CFFT/ICFFT source slices, including constexpr fixed FIR specialization |
