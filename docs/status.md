...
| FIR decimation | Experimental tensor-only `ondrix.fir_decimate` defines valid phase-zero decimation with static factor, increasing tap order, and result length `floor((N-K)/D)+1`; signed-Q15/full with signed frac30 accumulators of at least 32 bits has generic Ondsp lowering; factor-two i40 textual MLIR and portable i34 `.ox` profiles reach object+C differential execution; interpolation, nonzero phase, polyphase transforms, f32/Q31, Vector, and target consumers remain unsupported |
