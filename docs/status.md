...
| Algorithm transforms | Opt-in specialization for static rank-1 FIR samples using direct constant memref globals: zero-tap elimination and exact-modulo symmetric full-product pairing; reusable constant-sequence facts and subject-bound prefix-range planning are implemented, but saturating pairing remains disabled |
