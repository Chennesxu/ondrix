...
fixed full-output FIR, plus static four-/eight-point packed-Q15 CFFT.
...
| Packed Q15 butterfly | One closed scalar `i32` profile is executable: signed-Q15 complex values use imag-high/real-low packing, exact i33/Q30 cross-products, explicit nearest-even saturating product and output requantization, generic scalar lowering, and object+C differential execution; Vector, frontend, broader FFT, and OrtumCore selection remain unsupported |
| Packed Q15 CFFT | Experimental static four- and eight-point forward radix-2 DIT transforms implemented over matching `tensor<Nxi32>` values with natural input/output order, recursive even/odd stage decomposition, frozen Q15 twiddle tables, explicit per-stage scaling, generic scalar lowering, and object+C differential execution; an opt-in scalar-finalizer mode derives exact finite-precision `+1` and `-j` identities from constant twiddles while other constants and runtime twiddles retain generic i33 products; the `.ox` frontend exposes the same bounded profile as `tensor[complex_q15,4|8]`; inverse transforms, other sizes, Vector, and target execution remain unsupported |
| Object generation and C execution | Implemented for scalar and fixed-width Vector Q15/Q31 FIR-sample/full-output paths, valid Q15/f32 convolution/correlation, ordered scalar Q15/Q31/f32 streaming chunks, opt-in materialized Q15/Q31 streaming Vector reuse, the experimental f32 TDF-II plus uniform-Q fixed DF-II SOS cascades, the closed packed-Q15 scalar butterfly and static four-/eight-point CFFT profiles, and the `.ox` Q15/Q31/f32 dot/FIR plus packed-Q15 CFFT source slices, including constexpr fixed FIR specialization |
...
| Python-like `.ox` frontend | Experimental standalone Lexer/Parser/AST/Sema/MLIRGen pipeline for rank-1 Q15/Q31/f32 dot, FIR-sample, tensor-value valid FIR, static full-boundary FIR, and static four-/eight-point packed-Q15 CFFT kernels; buffers and tensors may have dynamic or static extent, while full boundary and CFFT are initially static; a fixed reduction or full-output FIR may carry inline constexpr right coefficients that lower to an immutable global and activate existing proof/specialization consumers; Q15 i40/frac30 and Q31 i64/frac62 accumulator policies, destination rounding/overflow, ordered f32 contraction, and the closed `complex_q15` stage-scaling profile are explicit, source locations reach MLIR, diagnostics are source-aware, and object+C execution is covered; dynamic full boundary/FFT planning, same padding, indexing, loops, multiple kernels, mutable output buffers, inferred accumulators, and stable source/ABI contracts remain planned |
