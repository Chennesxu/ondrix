...
| Recursive filters | Experimental tensor-only `ondrix.sos_filter_tdf2` for an explicit f32 transposed-direct-form-II cascade; coefficient order, additive denominator convention, per-section scale placement, `[z1,z2]` state, `off`/`fma`/`fast` contraction behavior, empty chunks, dynamic section guards, split-chunk state equivalence, and object+C execution are defined; fixed-point recurrence, Vector lowering, public buffer semantics, and stable frontend binding remain unsupported |
| Packed Q15 butterfly lowering | Experimental instruction selection only; no public emulator or ABI correctness claim |
| Object generation and C execution | Implemented for scalar and fixed-width Vector Q15/Q31 FIR-sample/full-output paths, ordered scalar Q15/Q31/f32 streaming chunks, opt-in materialized Q15/Q31 streaming Vector reuse, and the experimental f32 TDF-II SOS cascade |
