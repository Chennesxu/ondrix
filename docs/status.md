...
including immutable inline Q15 coefficients for statically sized FIR samples.
...
| Python-like `.ox` frontend | Experimental standalone Lexer/Parser/AST/Sema/MLIRGen pipeline for rank-1 signed-Q15 dot/FIR-sample kernels and f32 dot kernels; buffers may have dynamic or static extent, and a Q15 FIR may carry inline constexpr coefficients that lower to immutable memref globals and activate existing specialization; Q15 exact i40 update overflow plus destination rounding/overflow and f32 contraction are explicit, source locations reach MLIR, diagnostics are source-aware, and object+C execution is covered; general constexpr expressions, indexing, loops, multiple kernels, output buffers, inferred accumulators, and stable source/ABI contracts remain planned |
