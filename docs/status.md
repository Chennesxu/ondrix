...
| Packed Q15 CFFT | Experimental four-point forward radix-2 DIT transform implemented over `tensor<4xi32>` with natural input/output order, canonical `[1, 1, 1, -j]` twiddle schedule, four uses of the executable butterfly contract, explicit per-stage scaling, generic scalar lowering, and object+C differential execution; an opt-in scalar-finalizer mode derives exact finite-precision `+1` and `-j` identities from constant twiddles while runtime twiddles retain generic i33 products; other sizes, inverse transforms, Vector, frontend, and target execution remain unsupported |
| Object generation and C execution | Implemented for scalar and fixed-width Vector Q15/Q31 FIR-sample/full-output paths, ordered scalar Q15/Q31/f32 streaming chunks, opt-in materialized Q15/Q31 streaming Vector reuse, the experimental f32 TDF-II plus uniform-Q fixed DF-II SOS cascades, the closed packed-Q15 scalar butterfly and four-point CFFT profiles, and the `.ox` Q15/Q31/f32 dot/FIR-sample/valid full-output FIR source slices, including constexpr fixed FIR specialization |
