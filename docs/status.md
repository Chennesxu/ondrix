...
| FIR decimation | Experimental tensor-only `ondrix.fir_decimate` defines valid phase-zero decimation with static factor, increasing tap order, and result length `floor((N-K)/D)+1`; signed-Q15/full with signed frac30 accumulators of at least 32 bits has generic scalar and ordered fixed-Vector lowering; factor-two i40 textual MLIR and portable i34 `.ox` profiles reach object+C differential execution; nonzero phase, cross-output polyphase transforms, f32/Q31, and target consumers remain unsupported |
...
| Spectral magnitude | Experimental `ondrix.cx_magnitude` maps packed Q15 Hermitian bins to Q15 magnitudes through one exact i64 sum of squares and one explicit rounding boundary; `ondsp.sqrt_fixed` defines the all-integer square root (floor or nearest, no reachable ties, saturation at 32767, deterministic zero clamp of out-of-domain input) with a default bit-by-bit scalar lowering and an opt-in IEEE estimate-plus-correction lowering that stays bit-identical; both reach object+C differential execution behind an RFFT64 producer against an independent correction-looped reference; Vector and wider-width profiles fail closed |
| Fixed DCT-II | Experimental `ondrix.dct` fixes the direct matrix-form contract for extents 4 through 64: tie-guarded compile-time cosine tables, exact i64 row reductions, and one round-half-even boundary per output to the declared `frac = 14 - log2(N)` reading with provably unreachable saturation; generic scalar lowering and object+C differential execution are covered at 8 and 32; FFT-decomposed forms and target consumers remain unsupported |
...
| FIR design generation | Opt-in compile-time evaluation of `ondrix.window_hamming` and `ondrix.fir_design_windowed_sinc` (lowpass/highpass) into signed Q1.15 `arith.constant` tables under a fail-closed quantization tie guard with declared, counted saturation and provenance attributes; the generated constants feed the existing constant-coefficient proof and Vector machinery unchanged, with a design-to-object execution gate against independently derived golden taps |
