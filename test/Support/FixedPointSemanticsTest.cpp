...
bool expectEqual(llvm::StringRef name, const llvm::APInt &actual,
...
bool testQ15FullProducts() {
  ...
  passed &= expectEqual("max times max",
                        computeSignedFullProduct(signedValue(16, 32767),
                                                 signedValue(16, 32767)),
                        signedValue(32, 1073676289));
  passed &= expectEqual("min times min",
                        computeSignedFullProduct(signedValue(16, -32768),
...
bool testAccumulatorBoundaries() {
  ...
  bool passed = true;
  passed &= expectEqual(
      "add exact positive boundary",
      multiplyAccumulateSigned(maximum - 1, one, one,
                               AccumulatorUpdateOperation::Add,
                               AccumulatorOverflowMode::Saturate),
      maximum);
  passed &= expectEqual(
      "add positive saturation",
      multiplyAccumulateSigned(maximum, one, one,
                               AccumulatorUpdateOperation::Add,
                               AccumulatorOverflowMode::Saturate),
      maximum);
  passed &= expectEqual(
      "add negative saturation",
      multiplyAccumulateSigned(minimum, negativeOne, one,
                               AccumulatorUpdateOperation::Add,
                               AccumulatorOverflowMode::Saturate),
      minimum);
  passed &= expectEqual(
      "add exact negative boundary",
  ...
  passed &= expectEqual(
      ...
      maximum);
  passed &= expectEqual(
      "subtract positive saturation",
      multiplyAccumulateSigned(maximum, negativeOne, one,
                               AccumulatorUpdateOperation::Subtract,
                               AccumulatorOverflowMode::Saturate),
      maximum);
  passed &= expectEqual(
      "subtract negative saturation",
      multiplyAccumulateSigned(minimum, one, one,
                               AccumulatorUpdateOperation::Subtract,
                               AccumulatorOverflowMode::Saturate),
      minimum);
  passed &= expectEqual(
      "subtract positive wrap",
      multiplyAccumulateSigned(maximum, negativeOne, one,
                               AccumulatorUpdateOperation::Subtract,
                               AccumulatorOverflowMode::Wrap),
      minimum);
  passed &= expectEqual(
      "subtract negative wrap",
      multiplyAccumulateSigned(minimum, one, one,
                               AccumulatorUpdateOperation::Subtract,
                               AccumulatorOverflowMode::Wrap),
      maximum);
...
bool testAccumulatorChains() {
  ...
  for (unsigned i = 0; i != 3; ++i) {
    saturating = multiplyAccumulateSigned(
        saturating, one, one, AccumulatorUpdateOperation::Add,
        AccumulatorOverflowMode::Saturate);
    wrapping = multiplyAccumulateSigned(wrapping, one, one,
                                        AccumulatorUpdateOperation::Add,
                                        AccumulatorOverflowMode::Wrap);
  }
...
bool testSmallWidthExhaustive() {
  ...
  for (int64_t accumulator = accumulatorMinimum;
       accumulator <= accumulatorMaximum; ++accumulator) {
    for (int64_t lhs = operandMinimum; lhs <= operandMaximum; ++lhs) {
      for (int64_t rhs = operandMinimum; rhs <= operandMaximum; ++rhs) {
        ...
        for (AccumulatorUpdateOperation operation :
             {AccumulatorUpdateOperation::Add,
              AccumulatorUpdateOperation::Subtract}) {
          int64_t updated = operation == AccumulatorUpdateOperation::Add
                                ? accumulator + product
                                : accumulator - product;
          for (AccumulatorOverflowMode overflowMode :
               {AccumulatorOverflowMode::Wrap,
                AccumulatorOverflowMode::Saturate}) {
            int64_t expected =
                overflowMode == AccumulatorOverflowMode::Wrap
                    ? wrapSigned(updated, accumulatorWidth)
                    : std::clamp(updated, accumulatorMinimum,
                                 accumulatorMaximum);
            llvm::APInt actual = multiplyAccumulateSigned(
                signedValue(accumulatorWidth, accumulator),
                signedValue(operandWidth, lhs), signedValue(operandWidth, rhs),
