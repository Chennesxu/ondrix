...
// RUN: ondrix-compile %S/Inputs/q15_call_elementwise.ox | FileCheck %s --check-prefix=FIELDS
// RUN: not ondrix-compile %S/Inputs/invalid_callee_shift_amount.ox 2>&1 | FileCheck %s --check-prefix=CALLEERANGE
...
// Instantiation is a whole-value copy of the callee's call tree, so EVERY
// builtin parameter travels — this pins the two a field-by-field copier once
// silently dropped, and the range check on the second file fires at the
// callee with the callee's own values rather than against defaults.
// FIELDS-LABEL: func.func @q15_call_elementwise(
// FIELDS: ondrix.offset
// FIELDS-SAME: bias = 1024
// FIELDS: ondrix.shift
// FIELDS-SAME: amount = -2
// FIELDS-SAME: rounding = #ondsp.rounding<nearest_ties_positive>
// CALLEERANGE: invalid_callee_shift_amount.ox:2:10: error: shift amount must lie in [-15, 15]
