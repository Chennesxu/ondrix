...
extern void _mlir_ciface_fir_design_highpass_q15(MemRefI16 *, MemRefI16 *);

enum { kInputLength = 64, kTapCount = 9, kOutputLength = 56, kTrialCount = 16 };

/* Golden Q15 taps for the frozen windowed-sinc highpass profile
...
static int checkOutput(const int16_t *input, const char *label) {
  ...
  MemRefI16 output;
  _mlir_ciface_fir_design_highpass_q15(&output, &inputRef);
