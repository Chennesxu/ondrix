...
extern void _mlir_ciface_q15_fir_filter_constexpr(MemRefI16 *, MemRefI16 *);

static int16_t reference(const int16_t *input, int64_t index) {
  static const int16_t coefficients[] = {1024, -512, 256, 128};
  int64_t accumulator = 0;
  for (int64_t tap = 0; tap < 4; ++tap)
    accumulator += (int64_t)input[index + tap] * coefficients[tap];
...
int main(void) {
  ...
  MemRefI16 output;
  _mlir_ciface_q15_fir_filter_constexpr(&output, &inputRef);

  int failed = output.sizes[0] != 5;
