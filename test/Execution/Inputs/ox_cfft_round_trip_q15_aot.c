...
extern void _mlir_ciface_q15_cfft_round_trip(MemRefI32 *result, MemRefI32 *input);
...
static void transform4Strided(const int32_t input[8], unsigned offset, bool inverse,
                              int32_t output[4]) {
  const int32_t one = pack((struct Complex){INT16_MAX, 0});
  const int32_t quarterTurn =
      pack(inverse ? (struct Complex){0, INT16_MAX} : (struct Complex){0, INT16_MIN});
  int32_t s0, s1, s2, s3;
  ...
  butterfly(s2, s3, quarterTurn, &output[1], &output[3]);
}

static void transform8(const int32_t input[8], bool inverse, int32_t output[8]) {
  static const struct Complex forwardTwiddles[4] = {
      {INT16_MAX, 0},
      {23170, -23170},
      {0, INT16_MIN},
      {-23170, -23170},
  };
  static const struct Complex inverseTwiddles[4] = {
      {INT16_MAX, 0},
      {23170, 23170},
      {0, INT16_MAX},
      {-23170, 23170},
  };
  const struct Complex *twiddles = inverse ? inverseTwiddles : forwardTwiddles;
  int32_t even[4];
  int32_t odd[4];
  transform4Strided(input, 0, inverse, even);
  transform4Strided(input, 1, inverse, odd);
  for (unsigned k = 0; k < 4; ++k)
    butterfly(even[k], odd[k], pack(twiddles[k]), &output[k], &output[k + 4]);
}
...
int main(void) {
  static const struct Case cases[] = {
      ...
      {"mixed",
       {{12000, -7000},
        ...
        {-22000, -6000},
        {14000, 8000}}},
      {"rounding", {{1, -1}, {3, -3}, {5, -5}, {7, -7}, {9, -9}, {11, -11}, {13, -13}, {15, -15}}},
  };
  ...
  for (unsigned caseIndex = 0; caseIndex < sizeof(cases) / sizeof(cases[0]); ++caseIndex) {
    ...
    transform8(input, false, frequency);
    transform8(frequency, true, expected);

    MemRefI32 inputRef = {input, input, 0, {8}, {1}};
    MemRefI32 output;
    _mlir_ciface_q15_cfft_round_trip(&output, &inputRef);
    if (output.sizes[0] != 8) {
