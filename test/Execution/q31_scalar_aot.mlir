// RUN: ondrix-opt %s --convert-ondsp-fixed-to-scalar --convert-scf-to-cf --finalize-memref-to-llvm --convert-arith-to-llvm --convert-cf-to-llvm --convert-func-to-llvm --reconcile-unrealized-casts > %t.mlir
// RUN: ondrix-translate %t.mlir --mlir-to-llvmir > %t.ll
// RUN: llc -relocation-model=pic -filetype=obj %t.ll -o %t.o
// RUN: cc %S/Inputs/q31_scalar_aot.c %t.o -o %t
// RUN: %t
// RUN: cc %S/Inputs/q31_scalar_mismatch.c %t.o -o %t.mismatch
...
func.func @q31_reduce_full(%lhs: memref<?xi32>, %rhs: memref<?xi32>) -> i32 {
  %zero = ondsp.acc_zero : !ondsp.acc<storage = i64, frac = 62, signed, update_overflow = saturate>
...
func.func @q31_reduce_high_raw_q30(%lhs: memref<?xi32>, %rhs: memref<?xi32>) -> i32 {
  %zero = ondsp.acc_zero : !ondsp.acc<storage = i40, frac = 30, signed, update_overflow = saturate>
  %acc = ondsp.reduce_mac %zero, %lhs, %rhs {
    numeric = #ondsp.fixed<signed, storage = i32, frac = 31>,
    product = #ondsp.product<high_raw>
...
func.func @q31_repeat_full_raw_saturate(%lhs: i32, %rhs: i32, %count: index) -> i64 {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %zero = ondsp.acc_zero : !ondsp.acc<storage = i64, frac = 62, signed, update_overflow = saturate>
  %acc = scf.for %i = %c0 to %count step %c1
  ...
  %bits = builtin.unrealized_conversion_cast %acc : !ondsp.acc<storage = i64, frac = 62, signed, update_overflow = saturate> to i64
  return %bits : i64
}

func.func @q31_repeat_high_raw_saturate(%lhs: i32, %rhs: i32, %count: index) -> i64 {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %zero = ondsp.acc_zero : !ondsp.acc<storage = i40, frac = 30, signed, update_overflow = saturate>
  %acc = scf.for %i = %c0 to %count step %c1
  ...
  %bits = builtin.unrealized_conversion_cast %acc : !ondsp.acc<storage = i40, frac = 30, signed, update_overflow = saturate> to i40
