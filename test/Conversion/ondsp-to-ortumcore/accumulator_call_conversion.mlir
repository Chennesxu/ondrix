...
func.func @forward_accumulator(%acc: !ondsp.acc<storage = i40, frac = 30, signed>)
    -> !ondsp.acc<storage = i40, frac = 30, signed> {
  return %acc : !ondsp.acc<storage = i40, frac = 30, signed>
}

func.func @call_forward_accumulator(%acc: !ondsp.acc<storage = i40, frac = 30, signed>)
    -> !ondsp.acc<storage = i40, frac = 30, signed> {
  %0 = func.call @forward_accumulator(%acc) : (!ondsp.acc<storage = i40, frac = 30, signed>) -> !ondsp.acc<storage = i40, frac = 30, signed>
