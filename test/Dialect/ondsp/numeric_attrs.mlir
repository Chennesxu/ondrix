...
func.func @numeric_attrs(%x: i16, %y: f32) -> (i16, f32) {
  ...
  // CHECK: #ondsp.product<high_raw>
  %high = ondsp.assume_numeric %full {numeric = #ondsp.fixed<signed, storage = i16, frac = 15>, product = #ondsp.product<high_raw>} : (i16) -> i16
  ...
  %3 = ondsp.round_shift %high {scale = #ondsp.scale<pre_shift_left = 0, post_shift_right = 15, rounding = toward_negative, overflow = saturate, saturate_to = i16>} : (i16) -> i16
  return %3, %2 : i16, f32
}
