...
func.func @rejects_unestablished_state_rounding(
    %input: tensor<4xi16>, %coeffs: tensor<1x5xi16>,
    %scales: tensor<1xi16>, %state: tensor<1x2xi16>) {
  // expected-error @+1 {{state_rounding supports toward_negative, toward_zero, or nearest_even rounding}}
  %output, %next = ondrix.sos_filter_df2_fixed %input, %coeffs, %scales, %state {
    accumulator = !ondsp.acc<storage = i40, frac = 30, signed, update_overflow = saturate>,
    numeric = #ondsp.fixed<signed, storage = i16, frac = 15>,
    output_overflow = #ondsp.overflow<saturate>, output_rounding = #ondsp.rounding<nearest_even>,
...
func.func @rejects_unestablished_output_rounding(
    %input: tensor<4xi16>, %coeffs: tensor<1x5xi16>,
    %scales: tensor<1xi16>, %state: tensor<1x2xi16>) {
  // expected-error @+1 {{output_rounding supports toward_negative, toward_zero, or nearest_even rounding}}
  %output, %next = ondrix.sos_filter_df2_fixed %input, %coeffs, %scales, %state {
    accumulator = !ondsp.acc<storage = i40, frac = 30, signed, update_overflow = saturate>,
    numeric = #ondsp.fixed<signed, storage = i16, frac = 15>,
    output_overflow = #ondsp.overflow<saturate>,
